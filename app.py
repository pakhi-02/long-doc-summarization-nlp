...
# Page configuration
st.set_page_config(page_title="Long Document Summarizer", page_icon="📄", layout="wide")
...
chunk_method = st.sidebar.selectbox("Chunking Method", ["sentences", "paragraphs", "tokens"])
...
rag_query_presets = {
    "Default Overview": "main contributions key findings methodology conclusions",
    "Contributions": "novel contributions key innovations main ideas",
    "Methods": "methodology approach architecture training setup",
    "Results": "experimental results performance metrics ablation",
    "Limitations": "limitations failure cases constraints future work",
    "Custom": "",
}
selected_rag_preset = st.sidebar.selectbox(
    "RAG Query Preset",
    options=list(rag_query_presets.keys()),
    index=0,
    help="Choose a retrieval objective, or select Custom to write your own query",
)
custom_rag_query = st.sidebar.text_input(
    "Custom RAG Query",
    value="main contributions key findings methodology conclusions",
    help="Used only when preset is Custom",
    disabled=selected_rag_preset != "Custom",
)
effective_rag_query = (
    custom_rag_query.strip()
    if selected_rag_preset == "Custom"
    else rag_query_presets[selected_rag_preset]
)
if not effective_rag_query:
    effective_rag_query = "main contributions key findings methodology conclusions"
...
rag_top_k = st.sidebar.slider(
    ...
    value=5,
    step=1,
    help="Used only when method is 'rag'",
)
...
with tab1:
    ...
    if document_text and st.button("🚀 Generate Summary", type="primary"):
        ...
        with st.spinner(f"🤖 Summarizing with {selected_model}..."):
            try:
                ...
                result = summarizer.summarize_long_document(
                    ...
                    rag_query=effective_rag_query if summarization_method == "rag" else None,
                    retrieval_top_k=rag_top_k,
                )

                if summarization_method == "rag":
                    result["retrieval_preset"] = selected_rag_preset
        ...
        if result.get("method") == "rag":
            with st.expander("View RAG Retrieval Details"):
                st.markdown(f"**Preset:** {result.get('retrieval_preset', 'N/A')}")
                st.markdown(f"**Query:** {result.get('retrieval_query', '')}")
                retrieved_ids = result.get("retrieved_chunk_ids", [])
                retrieved_scores = result.get("retrieved_scores", [])
                if retrieved_ids:
                    for idx, score in zip(retrieved_ids, retrieved_scores):
                        st.write(f"Chunk {idx} (score: {score:.4f})")
                else:
                    st.write("No chunks were retrieved. Fallback summarization was used.")
        ...
        if st.button("Check Faithfulness"):
            with st.spinner("Analyzing faithfulness..."):
                faithfulness = calculate_faithfulness_score(
                    summary=result["final_summary"], original_text=document_text
                )
                ...
                with col2:
                    st.metric(
                        "Potential Hallucination Rate", f"{faithfulness['hallucination_ratio']:.2%}"
                    )
