...
class SummaryEvaluator:
    ...
    def compute_bertscore(
        self, summaries: List[str], references: List[str], lang: str = "en"
    ) -> Dict[str, Any]:
        """
        ...
        """
        P, R, F1 = bert_score(summaries, references, lang=lang, verbose=False)
    ...
    def evaluate(
        self, summary: str, reference: str, include_bertscore: bool = True
    ) -> Dict[str, float]:
        """
        ...
        results.update(
            {
                ...
                "compression_ratio": (
                    len(reference.split()) / len(summary.split()) if len(summary.split()) > 0 else 0
                ),
            }
        )
    ...
    def evaluate_batch(self, summaries: List[str], references: List[str]) -> Dict[str, Any]:
        """
...
def calculate_faithfulness_score(original_text: str, summary: str, sample_size: int = 5) -> float:
    """
